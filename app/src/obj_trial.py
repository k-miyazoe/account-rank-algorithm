...
def main():
    ...
    refund_probabilities_A = [1]
    refund_probabilities_B = [1]
    refund_probabilities_C = [1]
    agent_ratios_A = [0.9]
    agent_ratios_B = [0.09]
    agent_ratios_C = [0.01]

    output_folder_path = create_output_directory()
    create_params_file(output_folder_path, mail_probabilities_A, mail_probabilities_B, mail_probabilities_C, refund_probabilities_A, refund_probabilities_B, refund_probabilities_C, agent_ratios_A, agent_ratios_B, agent_ratios_C)
    ...
    for combination in parameter_combinations:
        ...
        simulation = Simulation(num_agents, csv_file_name, output_folder_path,agent_ratio_A, agent_ratio_B, agent_ratio_C)
        #1回のみ
        ...
        for i in range(10):
            ...
            a_msc_rank, b_msc_rank,c_msc_rank = average_msc(graph_rank)
            a_msc, b_msc,c_msc = average_msc(graph_normal)
            simulation_result = [refund_prob_A, refund_prob_B, refund_prob_C, 
                                 a_msc_rank, b_msc_rank,c_msc_rank,
                                 a_msc, b_msc,c_msc,
                                 ave_a_email_count_a, ave_b_email_count_a, ave_c_email_count_a,
                                 ave_a_email_count_n, ave_b_email_count_n, ave_c_email_count_n]
...
def create_params_file(dir_path, mail_probabilities_A, mail_probabilities_B, mail_probabilities_C, refund_probabilities_A, refund_probabilities_B, refund_probabilities_C, agent_ratios_A, agent_ratios_B, agent_ratios_C):
...
def create_output_csv(dir_path, csv_index):
    csv_header = ["返金確率A", "返金確率B", "返金確率C", 
                  "仮想通貨の平均所持量A(rank)", "仮想通貨の平均所持量B(rank)", "仮想通貨の平均所持量C(rank)",
                  "仮想通貨の平均所持量A(normal)", "仮想通貨の平均所持量B(normal)", "仮想通貨の平均所持量C(normal)",
                  "平均メール送信件数A(rank)", "平均メール送信件数B(rank)", "平均メール送信件数C(rank)",
                  "平均メール送信件数A(normal)", "平均メール送信件数B(normal)", "平均メール送信件数C(normal)"]
