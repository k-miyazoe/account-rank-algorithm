...
def main():
    ...
    output_file_name = create_output_csv(output_folder_path)
...
def create_output_csv(dir_path):
    csv_header = ["返金確率A", "返金確率B", "返金確率C", "仮想通貨の平均所持量A(後半)", "仮想通貨の平均所持量B(後半)", "仮想通貨の平均所持量C(後半)", "前半の平均メール送信件数A",
                  "前半の平均メール送信件数B", "前半の平均メール送信件数C", "後半の平均メール送信件数A", "後半の平均メール送信件数B", "後半の平均メール送信件数C","後半の平均メール送信件数A(なし)", "後半の平均メール送信件数B(なし)", "後半の平均メール送信件数C(なし)"]
